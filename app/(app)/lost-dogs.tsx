    ...
    <View style={styles.card}>
      <View style={styles.cardRow}>
        ...
        <View style={styles.cardInfo}>
          <Text style={styles.dogName}>{item.dog_name ?? 'כלב לא ידוע'} 🐾</Text>
          {item.owner_name ? (
            <Text style={styles.ownerName}>בעלים: {item.owner_name}</Text>
          ) : null}
          {item.note ? (
...
export default function LostDogs() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!coords) return;
      const { data } = await nearbyLostDogs(coords);
      setDogs(data);
    } finally {
      setLoading(false);
    }
    ...
    <View style={styles.screen}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>אין כלבים נעדרים בקרבתך 🐾</Text>
          }
        />
      )}
      ...
      <View style={styles.bottomBar}>
        ...
        >
          <Text style={styles.alertButtonText}>🚨 דווח על כלב נעדר</Text>
        </Pressable>
      </View>
    </View>
  );
