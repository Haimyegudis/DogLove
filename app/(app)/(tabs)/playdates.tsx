...
export default function Playdates() {
  ...
  const statusLabel: Record<string, string> = { pending: t('playdates.statusPending'), accepted: t('playdates.statusAccepted'), declined: t('playdates.statusDeclined') };

  const load = useCallback(() => {
    listIncoming().then(({ data, error }) => { if (error) { Alert.alert(t('playdates.error'), error); return; } setIncoming(data); });
    listOutgoing().then(({ data, error }) => { if (error) { Alert.alert(t('playdates.error'), error); return; } setOutgoing(data); });
  }, []);
  ...
  async function respond(requestId: string, accept: boolean) {
    ...
    if (error) { Alert.alert(t('playdates.error'), error); return; }
    load();
  }
    ...
    <DogParkBackground>
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          <Pressable onPress={() => router.push('/(app)/browse')} accessibilityRole="button" accessibilityLabel={t('playdates.browse')} style={[styles.browseBtn, shadow.soft]}>
            <Text style={styles.browseText}>{t('playdates.browse')}</Text>
          </Pressable>

          <Text style={styles.section}>{t('playdates.incoming')}</Text>
          {incoming.length === 0 ? <Text style={styles.empty}>{t('playdates.noIncoming')}</Text> : incoming.map((r) => (
            <View key={r.request_id} style={styles.reqWrap}>
              ...
              {r.status === 'pending' && (
                <View style={styles.actions}>
                  <Pressable onPress={() => respond(r.request_id, true)} accessibilityRole="button" accessibilityLabel={t('playdates.accept')} style={[styles.act, styles.accept]}><Text style={styles.actText}>{t('playdates.accept')}</Text></Pressable>
                  <Pressable onPress={() => respond(r.request_id, false)} accessibilityRole="button" accessibilityLabel={t('playdates.decline')} style={[styles.act, styles.decline]}><Text style={styles.actTextDark}>{t('playdates.decline')}</Text></Pressable>
                </View>
              )}
            </View>
          ))}

          <Text style={styles.section}>{t('playdates.outgoing')}</Text>
          {outgoing.length === 0 ? <Text style={styles.empty}>{t('playdates.noOutgoing')}</Text> : outgoing.map((r) => (
