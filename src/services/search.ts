...
export async function searchUsers(gender: string | null, minAge: number, maxAge: number, city = '') {
  const { data, error } = await supabase.rpc('search_users', { p_gender: gender, p_min_age: minAge, p_max_age: maxAge, p_city: city });
